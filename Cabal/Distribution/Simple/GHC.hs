...
buildLib :: Verbosity -> Bool -> Cabal.Flag (Maybe Int)
                  -> PackageDescription -> LocalBuildInfo
                  -> Library            -> ComponentLocalBuildInfo -> IO ()
buildLib = buildOrReplLib False

replLib :: Verbosity -> Cabal.Flag (Maybe Int)
                  -> PackageDescription -> LocalBuildInfo
                  -> Library            -> ComponentLocalBuildInfo -> IO ()
replLib v = buildOrReplLib True v False

buildOrReplLib :: Bool -> Verbosity  -> Bool -> Cabal.Flag (Maybe Int)
               -> PackageDescription -> LocalBuildInfo
               -> Library            -> ComponentLocalBuildInfo -> IO ()
buildOrReplLib forRepl verbosity typecheckOnly' numJobs pkg_descr lbi lib clbi = do
  let uid = componentUnitId clbi
      ...
      typecheckOnly = typecheckOnly' || componentIsIndefinite clbi
  ...
  let cObjs       = map (`replaceExtension` objExtension) (cSources libBi)
      baseOpts    = componentGhcOptions verbosity typecheckOnly lbi libBi clbi libTargetDir
  ...
  unless (forRepl || null (allLibModules lib clbi)) $
    do let vanilla = whenVanillaLib forceVanillaLib (runGhcProg vanillaOpts)
       ...
       unless typecheckOnly $ whenProfLib (runGhcProg profOpts)

  -- build any C sources
  unless (typecheckOnly || null (cSources libBi)) $ do
  ...
  unless typecheckOnly . ifReplLib $ do
    when (null (allLibModules lib clbi)) $ warn verbosity "No exposed modules"
    ifReplLib (runGhcProg replOpts)

  -- link:
  unless (typecheckOnly || forRepl) $ do
...
buildFLib
  :: Verbosity          -> Bool -> Cabal.Flag (Maybe Int)
  -> PackageDescription -> LocalBuildInfo
  -> ForeignLib         -> ComponentLocalBuildInfo -> IO ()
buildFLib v typecheckOnly njobs pkg lbi = gbuild v typecheckOnly njobs pkg lbi . GBuildFLib

replFLib
  :: Verbosity          -> Cabal.Flag (Maybe Int)
  -> PackageDescription -> LocalBuildInfo
  -> ForeignLib         -> ComponentLocalBuildInfo -> IO ()
replFLib  v njobs pkg lbi = gbuild v False njobs pkg lbi . GReplFLib
...
buildExe v typecheckOnly njobs pkg lbi = gbuild v typecheckOnly njobs pkg lbi . GBuildExe
replExe
  :: Verbosity          -> Cabal.Flag (Maybe Int)
  -> PackageDescription -> LocalBuildInfo
  -> Executable         -> ComponentLocalBuildInfo -> IO ()
replExe  v njobs pkg lbi = gbuild v False njobs pkg lbi . GReplExe
...
gbuild :: Verbosity          -> Bool -> Cabal.Flag (Maybe Int)
       -> PackageDescription -> LocalBuildInfo
       -> GBuildMode         -> ComponentLocalBuildInfo -> IO ()
gbuild verbosity typecheckOnly numJobs pkg_descr lbi bm clbi = do
  (ghcProg, _) <- requireProgram verbosity ghcProgram (withPrograms lbi)
  ...
  -- build executables
      baseOpts   = (componentGhcOptions verbosity typecheckOnly lbi bnfo clbi tmpDir)
                    `mappend` mempty {
                      ghcOptMode         = toFlag GhcModeMake,
  ...
  when (not typecheckOnly && compileForTH) $
  ...
  -- build any C sources
  unless (null cSrcs || typecheckOnly) $ do
   info verbosity "Building C Sources..."
   sequence_
     [ do let baseCcOpts    = Internal.componentCcGhcOptions verbosity implInfo
...
libAbiHash verbosity _pkg_descr lbi lib clbi = do
  ...
  let
      ...
      platform    = hostPlatform lbi
      vanillaArgs0 =
        (componentGhcOptions verbosity False lbi libBi clbi (componentBuildDir lbi clbi))
        `mappend` mempty {
...
componentGhcOptions :: Verbosity -> Bool -> LocalBuildInfo
                    -> BuildInfo -> ComponentLocalBuildInfo -> FilePath
                    -> GhcOptions
componentGhcOptions verbosity typecheckOnly lbi =
  Internal.componentGhcOptions verbosity typecheckOnly implInfo lbi
  where
    comp     = compiler lbi
    implInfo = getImplInfo comp
